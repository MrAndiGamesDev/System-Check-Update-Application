...
class AppWindow(Gtk.Window):
    ...
    def show_os_popup(self, os_name):
        ...
        dialog.format_secondary_text(
            ...
            "Report bugs on Discord: mrandi.games.dev"
        )
        dialog.run()
        dialog.destroy()
    ...
    def append_to_log(self, message):
        ...
        timestamp = time.strftime("[%Y-%m-%d %H:%M:%S]")  # Add a timestamp for clarity
        formatted_message = f"{timestamp} {message}\n"

        # Append the formatted message to the text buffer
        end_iter = self.log_buffer.get_end_iter()
        self.log_buffer.insert(end_iter, formatted_message)

        # Scroll to the end of the log view
